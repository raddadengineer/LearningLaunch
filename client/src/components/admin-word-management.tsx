...
export function AdminWordManagement() {
    ...
    return (
      <Card className="rounded-2xl">
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={handleLogin} className="w-full bg-red-500 hover:bg-red-600 text-white font-bold">
            Login
          </Button>
        </CardContent>
      </Card>
  ...
  if (isLoading) {
    return <p className="text-gray-600 p-4">Loading words...</p>;
  }
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="manage" className="w-full">
            ...
            {filteredWords.map((word) => (
              <Card key={word.id}>
                ...
                <CardContent className="space-y-3">
                  <div className="aspect-square w-full bg-gray-100 rounded-lg overflow-hidden max-h-40">
                    <img
                      src={word.imageUrl ?? ""}
                      alt={word.word}
                  ...
                  {editingWord?.id === word.id ? (
                    <div className="space-y-2">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <Button variant="outline" onClick={() => setEditingWord(null)} size="sm">
                          Cancel
                        </Button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <p className="text-xs text-gray-600 break-all line-clamp-2">{word.imageUrl}</p>
                      <div className="flex space-x-2">
                        <Button variant="outline" onClick={() => setEditingWord(word)} size="sm">
                          Edit
                        </Button>
                        <Button
                          variant="destructive"
        ...
        <TabsContent value="add" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              <CardTitle className="font-fredoka text-lg">Add New Word</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
              ...
              <Button onClick={handleAddWord} disabled={addWordMutation.isPending} className="w-full">
                Add Word
              </Button>
            </CardContent>
          </Card>
